...
GEN_MODEL     = os.environ.get("LAB04_GEN_MODEL", "gemma3:12b")   # any pulled Ollama model
...
def _ollama_reply(prompt_text):
    try:
        ...
        r = ollama.chat(
            ...
            messages=[
                {"role":"system","content":(
                    "You are a precise Pandas TA. Use ONLY the provided context. "
                    "If context is insufficient, say so briefly and suggest next steps. "
                    "Cite symbols explicitly. Provide exactly ONE concise, runnable example only if allowed."
                )},
...
def main():
    # 1) init collection
    try:
        col = get_or_create_collection(input_path=Path('data/pandas_help_corpus.json'), model=EMBED_MODEL)
